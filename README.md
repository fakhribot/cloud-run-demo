# **Deploy App Upload GCS ke Cloud Run**
...
1. **Buat variabel nama bucket** (Harus unik secara global).  
   ```
   export BUCKET_NAME=demo-upload-${GOOGLE_CLOUD_PROJECT}-xyz
   ```

2. **Buat Bucket**  
   ```
   gcloud storage buckets create gs://$BUCKET_NAME --location=asia-southeast2
   ```
...
Kita akan menggunakan perintah `gcloud run deploy --source .` yang akan otomatis membuild container dan mendeploynya.
...
2. **Berikan Role Storage Object Admin ke Service Account tersebut**  
   Ganti `[SERVICE_ACCOUNT_EMAIL]` dengan email yang didapat di atas.  
   ```
